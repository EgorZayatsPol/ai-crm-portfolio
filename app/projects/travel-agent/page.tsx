...
export default function TravelAgentPage() {
  return <main className="min-h-screen overflow-hidden">
    ...
    <article className="px-5 py-24 sm:px-8 lg:px-12 lg:py-32"><div className="mx-auto max-w-6xl space-y-28">
      ...
      <section><p className="eyebrow">How the agent works</p><h2 className="mt-4 text-3xl font-medium tracking-[-.05em] text-white sm:text-4xl">From a message to a useful shortlist.</h2><div className="mt-10 grid gap-3 md:grid-cols-4">{workflow.map(([number, title, description], index) => <article key={title} className="panel relative rounded-xl p-5"><span className="font-mono text-xs text-cyan-300">{number}</span>{index < workflow.length - 1 && <span className="absolute -right-3 top-1/2 z-10 hidden h-6 w-6 -translate-y-1/2 place-items-center rounded-full border border-white/10 bg-[#0d1421] text-slate-500 md:grid">→</span>}<h3 className="mt-7 text-lg font-medium text-white">{title}</h3><p className="mt-3 text-sm leading-6 text-slate-500">{description}</p></article>)}</div></section>

      <section id="telegram-demo"><div className="grid gap-10 lg:grid-cols-[.72fr_1.28fr]"><div><p className="eyebrow">Telegram demo</p><h2 className="mt-5 text-3xl font-medium tracking-[-.05em] text-white sm:text-4xl">A travel conversation,<br /><span className="text-slate-400">inside the familiar interface.</span></h2><p className="mt-5 max-w-md leading-7 text-slate-400">This static chat shows the intended bot experience. It does not send Telegram messages, make live web searches or call an external AI API.</p></div><div className="mx-auto w-full max-w-xl overflow-hidden rounded-[1.4rem] border border-slate-700 bg-[#0e1621] shadow-[0_24px_60px_rgba(0,0,0,.28)]"><div className="flex items-center gap-3 border-b border-white/10 bg-[#17212b] px-5 py-4"><div className="grid h-9 w-9 place-items-center rounded-full bg-cyan-300 font-mono text-xs font-bold text-slate-950">TA</div><div><p className="text-sm font-medium text-white">Travel Agent</p><p className="text-xs text-cyan-200">bot · online</p></div><span className="ml-auto text-slate-500">•••</span></div><div className="space-y-4 bg-[radial-gradient(circle_at_20%_20%,rgba(59,130,246,.08),transparent_28%),#0e1621] px-4 py-6 text-sm leading-6 sm:px-6"><div className="ml-auto max-w-[84%] rounded-2xl rounded-br-md bg-[#2b5278] px-4 py-3 text-slate-100">Find interesting things to do in Prague this weekend.<span className="ml-2 text-[10px] text-blue-200">18:42 ✓✓</span></div><div className="max-w-[88%] rounded-2xl rounded-bl-md bg-[#182533] px-4 py-3 text-slate-200">Sure! I&apos;m looking for activities and entertainment in Prague this weekend… <span className="ml-1 text-[10px] text-slate-500">18:42</span></div><div className="max-w-[92%] rounded-2xl rounded-bl-md bg-[#182533] px-4 py-3 text-slate-200"><p>Here are three ideas that fit a culture + evening-out mix:</p><div className="mt-3 space-y-3"><div className="rounded-xl border border-white/10 bg-[#213140] p-3"><p className="font-medium text-white">🎻 Vltava riverside concert</p><p className="mt-1 text-xs text-slate-400">Saturday · 19:30 · near Náplavka</p></div><div className="rounded-xl border border-white/10 bg-[#213140] p-3"><p className="font-medium text-white">🏰 Prague Castle after-hours tour</p><p className="mt-1 text-xs text-slate-400">Friday & Saturday · limited evening slots</p></div><div className="rounded-xl border border-white/10 bg-[#213140] p-3"><p className="font-medium text-white">🍺 Craft beer tasting in Karlín</p><p className="mt-1 text-xs text-slate-400">Small group · book ahead · indoor option</p></div></div><p className="mt-3">Would you prefer something more local and relaxed, or a bigger event?</p><span className="text-[10px] text-slate-500">18:43</span></div></div><div className="flex items-center gap-3 border-t border-white/10 bg-[#17212b] px-4 py-3 text-sm text-slate-500"><span>◉</span><span className="flex-1 rounded-full bg-[#0e1621] px-4 py-2">Write a message…</span><span className="text-cyan-300">➤</span></div></div></div></section>
