    ...
    <Link href={`/projects/${project.slug}`} className="project-card group">
      ...
      <div className="flex items-start justify-between gap-5 p-6 sm:p-7">
        <div>
          ...
          <h3 className="mt-4 text-2xl font-medium tracking-[-0.04em] text-white">{translate(project.title)}</h3>
          <p className="mt-3 max-w-md text-sm leading-6 text-slate-400">{translate(project.summary)}</p>
          <ul className="mt-6 flex flex-wrap gap-2">{project.tags.map((tag) => <li key={tag} className="tag">{translate(tag)}</li>)}</ul>
        </div>
